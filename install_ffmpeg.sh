...
if [ ! -e "$HOME/gnutls-3.7.0" ]; then
  ...
  cd gnutls-3.7.0
  LDFLAGS="-L${HOME}/compiled/lib" CFLAGS="-I${HOME}/compiled/include" LIBS="-lhogweed -lnettle -lgmp -lncrypt -lcrypt32 -lwsock32 -lws2_32 -lwinpthread" ./configure ${BUILD_OS:-} --prefix="$HOME/compiled" --enable-static --disable-shared --with-pic --with-included-libtasn1 --with-included-unistring --without-p11-kit --without-idn --without-zlib --disable-doc --disable-cxx --disable-tools --disable-hardware-acceleration --disable-guile --disable-libdane --disable-tests --disable-rpath --disable-nls
  make
  make install
  ...
  sed -i'' -e 's/-lgnutls/-lgnutls -lhogweed -lnettle -lgmp -lncrypt -lcrypt32 -lwsock32 -lws2_32 -lwinpthread/g' ~/compiled/lib/pkgconfig/gnutls.pc
fi
